...
def is_observable(indicator: str, period: pd.Period, vintage: date) -> bool:
    ...
    publication_date = end_of_period(period) + timedelta(days=PUBLICATION_LAG_DAYS[indicator])
...
def mask_for_vintage(
    ...
    for ind in indicators:
        observable = out[period_col].apply(lambda p: is_observable(ind, p, vintage))
